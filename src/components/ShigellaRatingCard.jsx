      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        {expanded && (
            <div>
              ...
              {/* Severe Complications Alert */}
              {(analysis.metrics?.hus || analysis.metrics?.hospitalized) && (
                  <div className="mb-4">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800">
                      <ul className="space-y-1">
                        {analysis.metrics.hus && (
                            <li className="text-sm text-red-900 dark:text-red-200 font-bold">
                              ⚠️ HEMOLYTIC UREMIC SYNDROME (HUS)
                            </li>
                        )}
                        {analysis.metrics.hospitalized && (
              ...
              {analysis.metrics?.symptomCounts && (
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      ...
                      {analysis.metrics.symptomCounts.bloodyStool > 0 && (
                          <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border-2 border-red-300">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Bloody Stool (Dysentery)</p>
                            <p className="text-2xl font-bold text-red-700 dark:text-red-300">
                              {analysis.metrics.symptomCounts.bloodyStool}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.tenesmus > 0 && (
                          <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Tenesmus (Straining)</p>
                            <p className="text-2xl font-bold text-orange-700 dark:text-orange-300">
                              {analysis.metrics.symptomCounts.tenesmus}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.reactiveArthritis > 0 && (
                          <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Reactive Arthritis</p>
                            <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">
                              {analysis.metrics.symptomCounts.reactiveArthritis}
                            </p>
                          </div>
                      )}
                    </div>
        ...
        {expanded && (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Based on 38 CFR 4.88b, Diagnostic Code 6334 - Shigella Infection. Causes dysentery (bloody diarrhea). Associated with Gulf War deployments. This analysis is for documentation purposes only. The VA makes all final rating determinations.
              </p>
            </div>
        )}
      </div>
