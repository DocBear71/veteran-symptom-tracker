...
import {IMMUNE_THROMBOCYTOPENIA_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
...
export default function ImmuneThrombocytopeniaRatingCard({
  ...
  const criteria = IMMUNE_THROMBOCYTOPENIA_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Immune
              Thrombocytopenia (ITP)</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC 7705 -
              38 CFR 4.117</p></div>
          </div>
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <ServiceConnectedBanner
                conditionKey="immuneThrombocytopenia"
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.plateletCount ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-xl font-bold ${metrics?.plateletCount ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.plateletCount ||
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Platelet
                      Count
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.bleedingEpisodes >
                      0 ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.bleedingEpisodes >
                        0 ?
                            'text-purple-600 dark:text-purple-400' :
                            'text-gray-400'}`}>{metrics?.bleedingEpisodes ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Bleeding
                    </div>
                  </div>
                  <div
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track platelet counts from lab work</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document bleeding episodes (bruising, nosebleeds)</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Record treatment (steroids, IVIG, splenectomy)</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.117, DC 7705 -
                Immune Thrombocytopenia. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
