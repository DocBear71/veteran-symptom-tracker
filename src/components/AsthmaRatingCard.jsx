        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Asthma</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 6602 -
                38 CFR 4.97</p></div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.attackCount >
                      0 ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.attackCount >
                    0 ?
                        'text-red-600 dark:text-red-400' :
                        'text-gray-400'}`}>{metrics?.attackCount || 0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Attacks
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.fev1Percent ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.fev1Percent ?
                        'text-purple-600 dark:text-purple-400' :
                        'text-gray-400'}`}>{metrics?.fev1Percent || '—'}%
                    </div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">FEV-1
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.usesInhaler ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.usesInhaler ?
                        'text-orange-600 dark:text-orange-400' :
                        'text-gray-400'}`}>{metrics?.usesInhaler ?
                        '✓' :
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Inhaler
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track pulmonary function test (PFT) results</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document asthma attacks requiring medical intervention</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Record daily/systemic corticosteroid use</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.97, DC 6602 -
                Asthma. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
