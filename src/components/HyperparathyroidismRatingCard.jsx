...
import {HYPERPARATHYROIDISM_CRITERIA} from '../utils/ratingLogic/index.js';
 ...
 *
 * Classic mnemonic: "Stones, Bones, Groans, and Moans"
 * - Stones: Kidney stones
 * - Bones: Bone pain, fractures, osteoporosis
 * - Groans: GI symptoms (constipation, nausea, abdominal pain)
 * - Moans: Neuropsychiatric (depression, confusion, fatigue)
 */
export default function HyperparathyroidismRatingCard({ analysis, expanded, onToggle }) {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Hyperparathyroidism</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7904 - 38 CFR 4.119</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.hasFracture ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.hasFracture ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>
                      {metrics?.hasFracture ? '✓' : '—'}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Fracture</div>
                  </div>
                  <div className="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg text-center">
                    <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">{metrics?.kidneySymptoms || 0}</div>
                    <div className="text-xs text-amber-700 dark:text-amber-300">Kidney</div>
                  </div>
                </div>
              </div>

              {/* Stones, Bones, Groans, Moans Breakdown */}
              <div>
                <h4 className="font-medium text-gray-900 dark:text-white mb-2 text-center">Symptom Categories</h4>
                <p className="text-xs text-gray-500 dark:text-gray-400 text-center mb-2">"Stones, Bones, Groans, and Moans"</p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  <div className="bg-teal-50 dark:bg-teal-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-teal-700 dark:text-teal-300">{metrics?.kidneySymptoms || 0}</div>
                    <div className="text-xs text-teal-600 dark:text-teal-400">Stones</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Kidney</div>
                  </div>
                  <div className="bg-amber-50 dark:bg-amber-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-amber-700 dark:text-amber-300">{metrics?.boneSymptoms || 0}</div>
                    <div className="text-xs text-amber-600 dark:text-amber-400">Bones</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Skeletal</div>
                  </div>
                  <div className="bg-lime-50 dark:bg-lime-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-lime-700 dark:text-lime-300">{metrics?.giSymptoms || 0}</div>
                    <div className="text-xs text-lime-600 dark:text-lime-400">Groans</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">GI</div>
                  </div>
                  <div className="bg-purple-50 dark:bg-purple-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-purple-700 dark:text-purple-300">{metrics?.neuroSymptoms || 0}</div>
                    <div className="text-xs text-purple-600 dark:text-purple-400">Moans</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Neuropsych</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  ...
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>60% requires objective hypercalcemia findings (labs, DEXA, or fracture)</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Residuals (kidney stones, fractures, vision) rated separately</span>
                  </li>
                </ul>
              </div>
