...
import {COPD_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
import UnderstandingYourRating from './UnderstandingYourRating';

/**
 * COPD Rating Card Component - Matches AsthmaRatingCard styling
 * DC 6604 - 38 CFR 4.97
 */
export default function COPDRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, rationale, evidenceGaps, metrics} = analysis;
  const criteria = COPD_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">COPD</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 6604 - 38 CFR 4.97</p>
            </div>
          </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span>FEV-1 is the key measure for obstructive lung disease</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>FEV-1/FVC ratio &lt;70% indicates airflow obstruction</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Post-bronchodilator results used for rating</span>
                  </li>
                </ul>
              </div>
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.97, DC 6604 - COPD. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
