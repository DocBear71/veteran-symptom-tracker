 ...
 */
export default function GoutRatingCard({ analysis, expanded, onToggle }) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-l-4 border-amber-500">
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${
                      metrics?.acuteAttacks > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'
                  }`}>
                    <div className={`text-2xl font-bold ${
                        metrics?.acuteAttacks > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'
                    }`}>
                      {metrics?.acuteAttacks || 0}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Acute Attacks</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${
              ...
              {/* Gout-Specific Information */}
              <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg border border-amber-200 dark:border-amber-800">
                <h4 className="font-semibold text-amber-900 dark:text-amber-200 mb-2 flex items-center gap-2">
                  <span>ℹ️</span>
                  Gout-Specific Documentation Tips
                </h4>
                <ul className="space-y-1">
                  <li className="text-sm text-amber-800 dark:text-amber-300 flex items-start gap-2">
                    <span className="text-amber-500 mt-0.5">•</span>
                    <span>Document each acute attack: date, joint affected, duration, severity</span>
                  </li>
                  <li className="text-sm text-amber-800 dark:text-amber-300 flex items-start gap-2">
                    <span className="text-amber-500 mt-0.5">•</span>
                    <span>Track serum uric acid levels from lab work</span>
                  </li>
                  <li className="text-sm text-amber-800 dark:text-amber-300 flex items-start gap-2">
                    <span className="text-amber-500 mt-0.5">•</span>
                    <span>Note any tophi (urate crystal deposits) if present</span>
                  </li>
                  <li className="text-sm text-amber-800 dark:text-amber-300 flex items-start gap-2">
                    <span className="text-amber-500 mt-0.5">•</span>
                    <span>Most commonly affects big toe (podagra), ankle, knee</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> {criteria?.disclaimer || 'Gout (DC 5017) is rated as degenerative arthritis per 38 CFR 4.71a. X-ray/imaging and lab values (uric acid) strengthen claims. For documentation purposes only.'}
              </div>
            </div>
        )}
      </div>
