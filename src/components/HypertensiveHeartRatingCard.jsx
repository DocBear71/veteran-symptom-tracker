...
import { HYPERTENSIVE_HEART_CRITERIA, getRatingRowColor, getRatingTextColor } from '../utils/ratingCriteria';
...
export default function HypertensiveHeartRatingCard({ analysis, expanded, onToggle }) {
  ...
  const criteria = HYPERTENSIVE_HEART_CRITERIA;
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Hypertensive Heart Disease</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7007 - 38 CFR 4.104</p>
            </div>
          </div>
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <ServiceConnectedBanner
                  conditionKey="hypertensiveHeart"
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.avgSystolicBP ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.avgSystolicBP ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>
                      {metrics?.avgSystolicBP || '—'}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Avg Systolic BP</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.hasHypertrophyOrDilatation ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.hasHypertrophyOrDilatation ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>
                      {metrics?.hasHypertrophyOrDilatation ? 'Yes' : 'No'}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">LVH/Dilatation</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 mt-3">
                  <div className="bg-orange-50 dark:bg-orange-900/20 p-2 rounded-lg text-center">
                    ...
                    <div className="text-xs text-orange-700 dark:text-orange-300">Dyspnea</div>
                  </div>
                  <div className="bg-amber-50 dark:bg-amber-900/20 p-2 rounded-lg text-center">
                    <div className="text-lg font-bold text-amber-600 dark:text-amber-400">{metrics?.orthopneaCount || 0}</div>
                    <div className="text-xs text-amber-700 dark:text-amber-300">Orthopnea/PND</div>
                  </div>
                  <div className="bg-teal-50 dark:bg-teal-900/20 p-2 rounded-lg text-center">
                    <div className="text-lg font-bold text-teal-600 dark:text-teal-400">{metrics?.edemaCount || 0}</div>
                    <div className="text-xs text-teal-700 dark:text-teal-300">Edema</div>
                  </div>
                  <div className="bg-rose-50 dark:bg-rose-900/20 p-2 rounded-lg text-center">
                    ...
                    <div className="text-xs text-rose-700 dark:text-rose-300">HF Hospitalizations</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Requires documented hypertension WITH cardiac involvement (not hypertension alone)</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Left ventricular hypertrophy (LVH) on echocardiogram supports 30% rating</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document both blood pressure readings AND heart failure symptoms</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Hypertension without cardiac changes should be rated under DC 7101</span></li>
                </ul>
              </div>
