...
import {CHRONIC_BRONCHITIS_CRITERIA} from '../utils/ratingLogic/';
 ...
 * DC 6600 - 38 CFR 4.97
 */
export default function ChronicBronchitisRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, rationale, evidenceGaps, metrics} = analysis;
  const criteria = CHRONIC_BRONCHITIS_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Chronic Bronchitis</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 6600 - 38 CFR 4.97</p>
            </div>
          </div>
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <ServiceConnectedBanner
                  conditionKey="chronicBronchitis"
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className={`p-3 rounded-lg text-center ${metrics?.fev1Percent ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.fev1Percent ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>
                      {metrics?.fev1Percent || '—'}%
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">FEV-1</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.fev1FvcRatio ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.fev1FvcRatio ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'}`}>
                      {metrics?.fev1FvcRatio || '—'}%
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">FEV-1/FVC</div>
                  </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span>Clinical definition: Productive cough 3+ months/year for 2+ consecutive years</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>VA rates by pulmonary function, not symptom frequency</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Post-bronchodilator PFT results used for rating</span>
                  </li>
                </ul>
              </div>
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.97, DC 6600 - Chronic Bronchitis. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
