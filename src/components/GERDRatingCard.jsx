...
import {GERD_CRITERIA} from '../utils/ratingCriteria';
import UnderstandingYourRating from './UnderstandingYourRating';

export default function GERDRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, rationale, evidenceGaps, metrics} = analysis;
  const criteria = GERD_CRITERIA;
  ...
  const getRatingRowColor = (percent, isSupported) => {
    ...
    if (percent >=
        100) return 'bg-red-100 dark:bg-red-900/30 border-red-300 dark:border-red-700';
    if (percent >=
        70) return 'bg-orange-100 dark:bg-orange-900/30 border-orange-300 dark:border-orange-700';
    if (percent >=
        50) return 'bg-amber-100 dark:bg-amber-900/30 border-amber-300 dark:border-amber-700';
    if (percent >=
        30) return 'bg-yellow-100 dark:bg-yellow-900/30 border-yellow-300 dark:border-yellow-700';
    if (percent >=
        10) return 'bg-green-100 dark:bg-green-900/30 border-green-300 dark:border-green-700';
  ...
  return (
      <div
          className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-l-4 border-lime-500">
        <button onClick={onToggle}
                className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3"><span
              className="text-2xl">🔥</span>
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">GERD
              (Hiatal Hernia)</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC 7346 -
              38 CFR 4.114</p></div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div
                  className="text-2xl font-bold text-lime-600 dark:text-lime-400">{supportedRating !==
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.refluxEpisodes >
                      0 ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.refluxEpisodes >
                        0 ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.refluxEpisodes ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Reflux
                      Episodes
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.regurgitation >
                      0 ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.regurgitation >
                        0 ?
                            'text-purple-600 dark:text-purple-400' :
                            'text-gray-400'}`}>{metrics?.regurgitation ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Regurgitation
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.dysphagia >
                      0 ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.dysphagia >
                    0 ?
                        'text-orange-600 dark:text-orange-400' :
                        'text-gray-400'}`}>{metrics?.dysphagia || 0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Dysphagia
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document heartburn, reflux, and regurgitation frequency</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track weight loss, anemia, or difficulty swallowing</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Record chest/arm pain episodes (substernal)</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.114, DC 7346 -
                Hiatal Hernia/GERD. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
