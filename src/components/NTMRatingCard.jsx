      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        {expanded && (
            <div>
              ...
              {/* Treatment Status */}
              {(analysis.metrics?.onTreatment || analysis.metrics?.activeDisease) && (
                  <div className="mb-4">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
                      Treatment Status
                    </h4>
                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
                      <ul className="space-y-1">
                        {analysis.metrics.activeDisease && (
                            <li className="text-sm text-blue-900 dark:text-blue-200 font-semibold">
                              Active NTM disease
                            </li>
                        )}
                        {analysis.metrics.onTreatment && (
                            <li className="text-sm text-blue-900 dark:text-blue-200">
                              On long-term antibiotic therapy ({analysis.metrics.monthsOnTreatment} months)
                            </li>
                        )}
                        {analysis.metrics.disseminated && (
                            <li className="text-sm text-red-900 dark:text-red-200 font-semibold">
                              ⚠️ Disseminated disease
                            </li>
                        )}
                      </ul>
                    </div>
              ...
              {analysis.metrics?.symptomCounts && (
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {analysis.metrics.symptomCounts.cough > 0 && (
                          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Chronic Cough</p>
                            <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">
                              {analysis.metrics.symptomCounts.cough}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.sputum > 0 && (
                          <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Sputum Production</p>
                            <p className="text-2xl font-bold text-green-700 dark:text-green-300">
                              {analysis.metrics.symptomCounts.sputum}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.hemoptysis > 0 && (
                          <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border-2 border-red-300">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Hemoptysis (Blood)</p>
                            <p className="text-2xl font-bold text-red-700 dark:text-red-300">
                              {analysis.metrics.symptomCounts.hemoptysis}
                            </p>
                          </div>
                      )}
        ...
        {expanded && (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Based on 38 CFR 4.88b, Diagnostic Code 6312 - Nontuberculous Mycobacterium. Related to but different from TB. Common species: MAC, M. abscessus, M. kansasii. Requires 12-24 months of multi-drug therapy. This analysis is for documentation purposes only. The VA makes all final rating determinations.
              </p>
            </div>
        )}
      </div>
