...
import { NTM_CRITERIA, getRatingRowColor, getRatingTextColor } from '../utils/ratingCriteria';
import UnderstandingYourRating from './UnderstandingYourRating';
import ServiceConnectedBanner from './ServiceConnectedBanner';
import {isRatingSupported} from '../utils/ratingUtils.js';
import MedicationCorrelation from './MedicationCorrelation';

export default function NTMRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) return null;

  const { supportedRating, ratingRationale, evidence, gaps, metrics } = analysis;
  const criteria = NTM_CRITERIA;
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Nontuberculous Mycobacteria (NTM)</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 6329 - 38 CFR 4.88b</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.symptomCounts?.cough > 0 ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.symptomCounts?.cough > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'}`}>{metrics?.symptomCounts?.cough || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Cough</div>
                  </div>
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.symptomCounts?.nightSweats > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.symptomCounts?.nightSweats > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.symptomCounts?.nightSweats || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Night Sweats</div>
                  </div>
                </div>
              </div>
              <MedicationCorrelation analysis={analysis} />

              {/* Supporting Evidence - string array from analyzeNTMLogs */}
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document chronic cough, fatigue, and weight loss</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Track pulmonary function test results</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>NTM often requires prolonged antibiotic treatment</span></li>
                </ul>
              </div>

              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.88b, Diagnostic Code 6329 - Nontuberculous Mycobacteria. This analysis is for documentation purposes only.
              </div>
            </div>
        )}
      </div>
