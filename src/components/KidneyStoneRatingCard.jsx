 ...
 * Displays VA rating analysis for Nephrolithiasis (DC 7508)
 * Based on 38 CFR 4.115b
 */
export default function KidneyStoneRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) return null;

  const { supportedRating, rationale, evidenceGaps, metrics } = analysis;
  const criteria = KIDNEY_STONES_CRITERIA;
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Kidney Stones (Nephrolithiasis)</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7508 - 38 CFR 4.115b</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.stoneEpisodes > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.stoneEpisodes > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.stoneEpisodes || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Stone Episodes</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.surgicalInterventions > 0 ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.surgicalInterventions > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.surgicalInterventions || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Interventions</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.recurrent ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.recurrent ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.recurrent ? '✓' : '—'}</div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document all stone passages and pain episodes</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Track surgical interventions (lithotripsy, stents, etc.)</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Record recurrence frequency and severity</span></li>
                </ul>
              </div>

              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.115b, Diagnostic Code 7508 - Nephrolithiasis. This analysis is for documentation purposes only.
              </div>
            </div>
        )}
      </div>
