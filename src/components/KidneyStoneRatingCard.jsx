...
export default function KidneyStonesRatingCard({ analysis, expanded, onToggle }) {
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              {ratingDetails && (
                  <div className="bg-teal-50 dark:bg-teal-900/20 p-4 rounded-lg">
                    ...
                    <p className="text-teal-800 dark:text-teal-300 mb-3">
                      {ratingDetails.summary}
                    </p>
                    <ul className="space-y-1">
                      {ratingDetails.criteriaDescription.map((item, idx) => (
                          <li key={idx} className="text-sm text-teal-700 dark:text-teal-400 flex items-start gap-2">
              ...
              {metrics && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {metrics.stoneEpisodes > 0 && (
                          <div className="bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg">
                            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                              {metrics.stoneEpisodes}
                            </div>
                      ...
                      {metrics.stonesPassed > 0 && (
                          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                            <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                              {metrics.stonesPassed}
                            </div>
                      ...
                      {metrics.proceduresLastYear > 0 && (
                          <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
                            <div className="text-2xl font-bold text-red-600 dark:text-red-400">
                              {metrics.proceduresLastYear}
                            </div>
                            <div className="text-xs text-red-700 dark:text-red-300">
                              Procedures (Past Year)
                            </div>
                          </div>
                      )}
                      {metrics.procedureTypes && metrics.procedureTypes.length > 0 && (
                          <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg col-span-2">
                            <div className="text-sm font-medium text-purple-700 dark:text-purple-300">
                              Procedures: {metrics.procedureTypes.join(', ')}
                            </div>
                          </div>
                      )}
                    </div>
