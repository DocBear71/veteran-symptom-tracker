...
import {SHOULDER_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
import UnderstandingYourRating from './UnderstandingYourRating';
import ServiceConnectedBanner from './ServiceConnectedBanner';
import MedicationCorrelation from './MedicationCorrelation';

export default function ShoulderRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, rationale, evidenceGaps, metrics} = analysis;
  const criteria = SHOULDER_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Shoulder
              Condition</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC
              5200-5203 - 38 CFR 4.71a</p></div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.romLimited ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.romLimited ?
                        'text-red-600 dark:text-red-400' :
                        'text-gray-400'}`}>{metrics?.flexion || '—'}°
                    </div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Flexion
                    </div>
                  </div>
                  <div
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document arm motion at shoulder level (90°)</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Note dominant vs non-dominant arm</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track pain with overhead activities</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.71a, DC 5200-5203
                - Shoulder. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
