...
import {MALIGNANT_ORAL_NEOPLASM_CRITERIA} from '../utils/ratingLogic/';
...
export default function MalignantOralNeoplasmRatingCard({
  ...
  const criteria = MALIGNANT_ORAL_NEOPLASM_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Malignant
              Oral Neoplasm</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC {criteria?.diagnosticCode ||
                '9911'} - 38 CFR 4.150</p></div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.diseaseStatus ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-lg font-bold ${metrics?.diseaseStatus ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.diseaseStatus ||
                        '—'}</div>
                    <div
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.functionalLoss ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.functionalLoss ?
                            'text-orange-600 dark:text-orange-400' :
                            'text-gray-400'}`}>{metrics?.functionalLoss ?
                        '✓' :
                        '—'}</div>
                    <div
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>100% rating during active malignancy or treatment</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>After treatment: rated on residuals</span>
                  </li>
                </ul>
              </div>
              <div
