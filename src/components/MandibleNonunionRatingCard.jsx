...
import {MANDIBLE_NONUNION_CRITERIA} from '../utils/ratingLogic/';
...
export default function MandibleNonunionRatingCard({
  ...
  const criteria = MANDIBLE_NONUNION_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Mandible
              Nonunion</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC {criteria?.diagnosticCode ||
                '9903'} - 38 CFR 4.150</p></div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.severity ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-lg font-bold ${metrics?.severity ?
                        'text-red-600 dark:text-red-400' :
                        'text-gray-400'}`}>{metrics?.severity || '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Severity
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.masticationLoss ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.masticationLoss ?
                            'text-purple-600 dark:text-purple-400' :
                            'text-gray-400'}`}>{metrics?.masticationLoss ?
                        '✓' :
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Mast.
                      Loss
                    </div>
                  </div>
                  <div
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Severity rated as severe, moderate, or slight</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document degree of mastication loss</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.150, DC 9903. For
