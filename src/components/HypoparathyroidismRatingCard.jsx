...
import { HYPOPARATHYROIDISM_CRITERIA } from '../utils/ratingCriteria';

/**
 * Hypoparathyroidism Rating Card - Gold Standard Version
 * DC 7905 - 38 CFR 4.119
 */
export default function HypoparathyroidismRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) return null;

  const { supportedRating, ratingRationale, gaps, metrics } = analysis;
  const criteria = HYPOPARATHYROIDISM_CRITERIA;
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Hypoparathyroidism</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7905 - 38 CFR 4.119</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.hasSeizures ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.hasSeizures ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>
                      {metrics?.hasSeizures ? '✓' : '—'}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Seizures</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.hasCataracts ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.hasCataracts ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400'}`}>
                      {metrics?.hasCataracts ? '✓' : '—'}
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Cataracts</div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-yellow-700 dark:text-yellow-300">{metrics?.neuromuscularSymptoms || 0}</div>
                    <div className="text-xs text-yellow-600 dark:text-yellow-400">Neuromuscular</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Cramps/Spasms</div>
                  </div>
                  <div className="bg-purple-50 dark:bg-purple-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-purple-700 dark:text-purple-300">{metrics?.neuropsychSymptoms || 0}</div>
                    <div className="text-xs text-purple-600 dark:text-purple-400">Neuropsych</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Mood/Memory</div>
                  </div>
                  <div className="bg-pink-50 dark:bg-pink-900/20 p-2 rounded text-center">
                    <div className="text-lg font-semibold text-pink-700 dark:text-pink-300">{metrics?.integumentarySymptoms || 0}</div>
                    <div className="text-xs text-pink-600 dark:text-pink-400">Integumentary</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Skin/Nails/Hair</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span>100% rating for 3 months after initial diagnosis</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>After 3 months, rate chronic residuals separately</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Seizures from hypocalcemia: Rate under DC 8910-8914</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Cataracts, kidney stones, CHF rated under respective DCs</span>
                  </li>
                </ul>
              </div>
