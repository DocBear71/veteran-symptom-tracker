...
import { HYPERTENSION_CRITERIA, getRatingRowColor, getRatingTextColor } from '../utils/ratingCriteria';
 ...
 */
export default function HypertensionRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) return null;

  const { supportedRating, rationale, evidenceGaps, metrics, ratingRationale, gaps } = analysis;
  const criteria = HYPERTENSION_CRITERIA;
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Hypertension</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7101 - 38 CFR 4.104</p>
            </div>
          </div>
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg text-center">
                    <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{metrics?.totalReadings || 0}</div>
                    <div className="text-xs text-blue-700 dark:text-blue-300">BP Readings</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.avgSystolic ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.avgSystolic ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.avgSystolic || '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Avg Systolic</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.avgDiastolic ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.avgDiastolic ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.avgDiastolic || '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Avg Diastolic</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.onMedication ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.onMedication ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.onMedication ? '✓' : '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Medication</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span>Take BP readings at same time daily</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>History of diastolic 100+ is key for higher ratings</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Document continuous medication requirement</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.104, DC 7101 - Hypertension. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
