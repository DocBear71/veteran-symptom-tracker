...
import {PERIPHERAL_NEUROPATHY_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
...
export default function PeripheralNeuropathyRatingCard({
  ...
  const criteria = PERIPHERAL_NEUROPATHY_CRITERIA;
  ...
  return (
      <div
          className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-l-4 border-indigo-500">
        <button onClick={onToggle}
                className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3"><span
              className="text-2xl">⚡</span>
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Peripheral
              Neuropathy</h3><p
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <ServiceConnectedBanner
                conditionKey="peripheralNeuropathy"
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.numbnessDays >
                      0 ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.numbnessDays >
                        0 ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.numbnessDays ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Numbness
                      Days
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.tingling >
                      0 ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.tingling > 0 ?
                            'text-purple-600 dark:text-purple-400' :
                            'text-gray-400'}`}>{metrics?.tingling || 0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Tingling
                      Days
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.muscleWeakness ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.muscleWeakness ?
                            'text-orange-600 dark:text-orange-400' :
                            'text-gray-400'}`}>{metrics?.muscleWeakness ?
                        '✓' :
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Weakness
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track numbness, tingling, and burning sensations</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document muscle weakness and atrophy</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Note which extremities are affected</span>
                  </li>
                </ul>
              </div>
              <div
