      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        {expanded && (
            <div>
              ...
              {/* Neuroinvasive Disease Alert */}
              {(analysis.metrics?.neuroinvasive || analysis.metrics?.acuteFlaccidParalysis) && (
                  <div className="mb-4">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-2">
                    ...
                    <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 border border-red-200 dark:border-red-800">
                      <ul className="space-y-1">
                        {analysis.metrics.acuteFlaccidParalysis && (
                            <li className="text-sm text-red-900 dark:text-red-200 font-bold">
                              ⚠️ ACUTE FLACCID PARALYSIS (Polio-like)
                            </li>
                        )}
                        {analysis.metrics.encephalitis && (
                            <li className="text-sm text-red-900 dark:text-red-200 font-semibold">
                              Encephalitis (Brain inflammation)
                            </li>
                        )}
                        {analysis.metrics.neuroinvasive && !analysis.metrics.encephalitis && !analysis.metrics.acuteFlaccidParalysis && (
                            <li className="text-sm text-red-900 dark:text-red-200">
                              Neuroinvasive disease documented
                            </li>
                        )}
                      </ul>
                    </div>
              ...
              {analysis.metrics?.symptomCounts && (
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {analysis.metrics.symptomCounts.weakness > 0 && (
                          <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border-2 border-red-300">
                            ...
                            <p className="text-2xl font-bold text-red-700 dark:text-red-300">
                              {analysis.metrics.symptomCounts.weakness}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.paralysis > 0 && (
                          <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border-2 border-red-500">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Paralysis (AFP)</p>
                            <p className="text-2xl font-bold text-red-700 dark:text-red-300">
                              {analysis.metrics.symptomCounts.paralysis}
                            </p>
                          </div>
                      )}
                      {analysis.metrics.symptomCounts.fatigue > 0 && (
                          <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Chronic Fatigue</p>
                      ...
                      {analysis.metrics.symptomCounts.tremors > 0 && (
                          <div className="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Tremors</p>
                            <p className="text-2xl font-bold text-orange-700 dark:text-orange-300">
                              {analysis.metrics.symptomCounts.tremors}
                            </p>
                          </div>
                      )}
                    </div>
        ...
        {expanded && (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Based on 38 CFR 4.88b, Diagnostic Code 6335 - West Nile Virus. Mosquito-borne illness. Post-West Nile syndrome can cause chronic fatigue and weakness lasting years. This analysis is for documentation purposes only. The VA makes all final rating determinations.
              </p>
            </div>
        )}
      </div>
