...
import { WEST_NILE_CRITERIA, getRatingRowColor, getRatingTextColor } from '../utils/ratingCriteria';
...
export default function WestNileRatingCard({ analysis, expanded, onToggle }) {
  ...
  const criteria = WEST_NILE_CRITERIA;
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">West Nile Virus</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 6335 - 38 CFR 4.88b</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.symptomCounts?.neurologicalSymptoms > 0 ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.symptomCounts?.neurologicalSymptoms > 0 ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.symptomCounts?.neurologicalSymptoms || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Neuro Symptoms</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document fever, headache, and muscle weakness</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Track neurological symptoms (encephalitis, meningitis residuals)</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Neurological residuals may warrant higher ratings</span></li>
                </ul>
              </div>

              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.88b, Diagnostic Code 6335 - West Nile Virus. This analysis is for documentation purposes only.
              </div>
            </div>
        )}
      </div>
