...
import { LEUKEMIA_CRITERIA } from '../utils/ratingCriteria';

export default function LeukemiaRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) {
    return null;
  }

  const { supportedRating, rationale, evidenceGaps, metrics } = analysis;
  const criteria = LEUKEMIA_CRITERIA;
  const ratingDetails = criteria.ratings.find(r => r.percent === supportedRating);

  return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-l-4 border-purple-600">
        {/* Header */}
        <button
            onClick={onToggle}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-purple-800 dark:text-purple-300">
                          VA assigns 100% rating for all active leukemia treatment. Rating is reduced
                          after remission based on residual symptoms.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {metrics && (
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      {metrics.treatmentTypes && metrics.treatmentTypes.size > 0 && (
                          <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg col-span-2">
                            <div className="text-sm font-medium text-purple-700 dark:text-purple-300">
                              Treatments: {Array.from(metrics.treatmentTypes).join(', ')}
                            </div>
                          </div>
                      )}
                    </div>
