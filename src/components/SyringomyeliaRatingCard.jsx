...
import {SYRINGOMYELIA_CRITERIA} from '../utils/ratingLogic/index';
import RatingEnhancementsDisplay from './RatingEnhancementsDisplay';

/**
 * Syringomyelia Rating Card Component - Gold Standard Version
 * DC 8024 - 38 CFR 4.124a
 */
export default function SyringomyeliaRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, ratingRationale, gaps, metrics} = analysis;
  const criteria = SYRINGOMYELIA_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Syringomyelia</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 8024 - 38 CFR 4.124a</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.sensoryLogs > 0 ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.sensoryLogs > 0 ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.sensoryLogs || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Sensory</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.weaknessLogs > 0 ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.weaknessLogs > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.weaknessLogs || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Weakness</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.painLogs > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.painLogs > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.painLogs || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Pain</div>
                  </div>
                </div>
              </div>

              {/* Dissociated Sensory Loss Indicator */}
              {metrics?.hasDissociatedSensoryLoss && (
                  <div className="bg-violet-50 dark:bg-violet-900/20 p-3 rounded-lg border border-violet-200 dark:border-violet-800 text-center">
                    <span className="text-sm text-violet-800 dark:text-violet-200 font-medium">
                      ✓ Dissociated Sensory Loss Documented - Characteristic finding (temp/pain insensitivity with preserved touch)
                    </span>
                  </div>
              )}
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span><strong>Dissociated sensory loss</strong> (loss of pain/temperature with preserved touch) is pathognomonic</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>"Cape distribution" sensory loss affects shoulders, arms, and upper back</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Document burns or injuries resulting from temperature/pain insensitivity</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>MRI showing syrinx size and location strengthens claim</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.124a, DC 8024 - Syringomyelia. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
