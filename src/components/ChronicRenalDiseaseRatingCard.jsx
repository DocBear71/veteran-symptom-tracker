 ...
 * Displays VA rating analysis for Chronic Kidney Disease (DC 7541)
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Chronic Kidney Disease</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7541 - 38 CFR 4.115a</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.ckdStage ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.ckdStage ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.ckdStage || '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">CKD Stage</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.onDialysis ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.onDialysis ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.onDialysis ? '✓' : '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Dialysis</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.gfr ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.gfr ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.gfr || '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">eGFR</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Track eGFR and creatinine levels over time</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document dialysis treatments if applicable</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Record symptoms: fatigue, swelling, appetite changes</span></li>
                </ul>
              </div>

              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.115a, Diagnostic Code 7541 - Renal Disease. This analysis is for documentation purposes only.
              </div>
            </div>
        )}
      </div>
