...
import {TOOTH_LOSS_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Tooth
              Loss</h3><p
                className="text-sm text-gray-600 dark:text-gray-400">DC {criteria?.diagnosticCode ||
                '9913'} - 38 CFR 4.150</p></div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.teethMissing >
                      0 ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.teethMissing >
                        0 ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.teethMissing ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Teeth
                      Missing
                    </div>
                  </div>
                  <div
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.prosthesisUsed ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.prosthesisUsed ?
                            'text-orange-600 dark:text-orange-400' :
                            'text-gray-400'}`}>{metrics?.prosthesisUsed ?
                        '✓' :
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Prosthesis
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document number of teeth lost due to service</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track masticatory surface loss</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Note if prosthesis is possible or not</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.150, DC 9913. For
