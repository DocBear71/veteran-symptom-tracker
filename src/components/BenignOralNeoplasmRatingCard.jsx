...
import {BENIGN_ORAL_NEOPLASM_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
import UnderstandingYourRating from './UnderstandingYourRating';
import ServiceConnectedBanner from './ServiceConnectedBanner';

export default function BenignOralNeoplasmRatingCard({
                                                       analysis,
                                                       expanded,
                                                       onToggle,
                                                     }) {
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Benign
              Oral Neoplasm</h3><p
        ...
        {expanded && (
            <div className="px-6 pb-6 space-y-6">
              ...
              <ServiceConnectedBanner
                  conditionKey="benignOralNeoplasm"
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.growthStatus ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-lg font-bold ${metrics?.growthStatus ?
                        'text-purple-600 dark:text-purple-400' :
                        'text-gray-400'}`}>{metrics?.growthStatus || '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Status
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.functionalImpact ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.functionalImpact ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.functionalImpact ?
                        '✓' :
                        '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Func.
                      Impact
                    </div>
                  </div>
                  <div
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Rated on residual functional impairment</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document impact on speech, chewing, swallowing</span>
                  </li>
                </ul>
              </div>
              <div
                  className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.150. For
