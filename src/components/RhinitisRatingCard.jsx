...
import {RHINITIS_CRITERIA, getRatingRowColor, getRatingTextColor} from '../utils/ratingCriteria';
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3"><span
            ...
            <div className="text-left"><h3
                className="font-semibold text-lg text-gray-900 dark:text-white">Allergic/Vasomotor
              Rhinitis</h3><p
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div><h4
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.nasalObstruction >
                      0 ?
                          'bg-red-50 dark:bg-red-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div
                        className={`text-2xl font-bold ${metrics?.nasalObstruction >
                        0 ?
                            'text-red-600 dark:text-red-400' :
                            'text-gray-400'}`}>{metrics?.nasalObstruction ||
                        0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Obstruction
                      Days
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.polyps ?
                          'bg-purple-50 dark:bg-purple-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.polyps ?
                        'text-purple-600 dark:text-purple-400' :
                        'text-gray-400'}`}>{metrics?.polyps ? '✓' : '—'}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Polyps
                    </div>
                  </div>
                  <div
                      className={`p-3 rounded-lg text-center ${metrics?.discharge >
                      0 ?
                          'bg-orange-50 dark:bg-orange-900/20' :
                          'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.discharge >
                    0 ?
                        'text-orange-600 dark:text-orange-400' :
                        'text-gray-400'}`}>{metrics?.discharge || 0}</div>
                    <div
                        className="text-xs text-gray-600 dark:text-gray-400">Discharge
                      Days
                    </div>
                  </div>
                </div>
              ...
              <UnderstandingYourRating
                  diagnosticCode={criteria.diagnosticCode || '6522'}
              ...
              <div
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Document nasal obstruction (50%+ one side or complete both sides)</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Note presence of nasal polyps</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span><span>Track antihistamine/steroid spray usage</span>
                  </li>
                </ul>
              </div>
              <div
