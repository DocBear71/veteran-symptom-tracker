 ...
 * DC 8017 - 38 CFR 4.124a
 * Note: ALS is presumptively service-connected for all veterans
 */
export default function ALSRatingCard({analysis, expanded, onToggle}) {
  if (!analysis || !analysis.hasData) return null;
  const {supportedRating, ratingRationale, gaps, metrics} = analysis;
  const criteria = ALS_CRITERIA;
        ...
        <button onClick={onToggle}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">ALS (Amyotrophic Lateral Sclerosis)</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 8017 - 38 CFR 4.124a</p>
            </div>
          </div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800 text-center">
                <span className="text-sm text-blue-800 dark:text-blue-200 font-medium">
                  ⚠️ ALS is presumptively service-connected for all veterans
                </span>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className={`p-3 rounded-lg text-center ${metrics?.bulbarLogs > 0 ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.bulbarLogs > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.bulbarLogs || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Bulbar</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.respiratoryLogs > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.respiratoryLogs > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.respiratoryLogs || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Respiratory</div>
                  </div>
                </div>
              </div>

              {/* Involvement Indicators */}
              {(metrics?.hasBulbarInvolvement || metrics?.hasRespiratoryInvolvement) && (
                  <div className="flex flex-wrap justify-center gap-2">
                    {metrics.hasBulbarInvolvement && (
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    ...
                    <span><strong>ALS is presumptively service-connected</strong> for all veterans regardless of when diagnosed</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Minimum 30% rating with confirmed diagnosis</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Progressive disease - ratings should be reviewed as condition advances</span>
                  </li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2">
                    <span className="text-blue-500 mt-0.5">•</span>
                    <span>Document FVC (forced vital capacity) for respiratory status</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.124a, DC 8017 - Amyotrophic Lateral Sclerosis. For documentation purposes only.
              </div>
            </div>
        )}
      </div>
