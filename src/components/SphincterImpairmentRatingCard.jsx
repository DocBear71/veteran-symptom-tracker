 ...
 * Displays VA rating analysis for Fecal Incontinence (DC 7332)
 * Based on 38 CFR 4.114
 */
export default function SphincterImpairmentRatingCard({ analysis, expanded, onToggle }) {
  if (!analysis || !analysis.hasData) return null;

  const { supportedRating, rationale, evidenceGaps, metrics } = analysis;
  const criteria = SPHINCTER_IMPAIRMENT_CRITERIA;
  ...
  const isRatingSupported = (ratingPercent) => {
    return numericRating === ratingPercent;
  };
        ...
        <button onClick={onToggle} className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-lg text-gray-900 dark:text-white">Sphincter Impairment (Fecal Incontinence)</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">DC 7332 - 38 CFR 4.114</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="text-2xl font-bold text-teal-600 dark:text-teal-400">{supportedRating !== null ? `${supportedRating}%` : 'N/A'}</div>
            ...
            <div className="px-6 pb-6 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg text-center">
                    <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{metrics?.totalLogs || metrics?.episodes || 0}</div>
                    <div className="text-xs text-blue-700 dark:text-blue-300">Total Logs</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.episodes > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-2xl font-bold ${metrics?.episodes > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{metrics?.episodes || 0}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Episodes</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.episodeRate ? 'bg-orange-50 dark:bg-orange-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-xl font-bold ${metrics?.episodeRate ? 'text-orange-600 dark:text-orange-400' : 'text-gray-400'}`}>{metrics?.episodeRate || '—'}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-400">Episode Rate</div>
                  </div>
                  <div className={`p-3 rounded-lg text-center ${metrics?.mostCommonFrequency ? 'bg-purple-50 dark:bg-purple-900/20' : 'bg-gray-50 dark:bg-gray-700/30'}`}>
                    <div className={`text-sm font-bold ${metrics?.mostCommonFrequency ? 'text-purple-600 dark:text-purple-400' : 'text-gray-400'}`}>{metrics?.mostCommonFrequency || '—'}</div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="space-y-1">
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Document frequency and severity of incontinence episodes</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Track use of protective garments or pads</span></li>
                  <li className="text-sm text-blue-800 dark:text-blue-300 flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span><span>Note any constant leakage or colostomy needs</span></li>
                </ul>
              </div>

              {/* Disclaimer */}
              <div className="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-400">
                <strong>Important:</strong> Based on 38 CFR 4.114, Diagnostic Code 7332 - Rectum and Anus, Impairment of Sphincter Control. This analysis is for documentation purposes only.
              </div>
            </div>
        )}
      </div>
